...
export async function scrapeLinkedIn(
  ...
  try {
    ...
    // geoId 104508036 = Czechia; &start=N paginates at 25 results per page
    const baseUrl = `https://www.linkedin.com/jobs/search/?keywords=${q}&location=Czechia&geoId=104508036${expParam}`;

    for (let pageNum = 0; pageNum < MAX_PAGES; pageNum++) {
      const pageUrl = pageNum === 0 ? baseUrl : `${baseUrl}&start=${pageNum * 25}`;
      ...
      if (cards.length === 0) break; // no more results on this page

      // Deep search uses all cards on the page; normal search caps at 10
      const pageCards = deepSearch ? cards : cards.slice(0, 10);
      ...
      if (deepSearch) {
        ...
        const pageSourceUrls = seeds.map((s) => s.sourceUrl);
